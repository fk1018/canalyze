...
import { uploadCanMessage } from '../../src/services/upload-service.js'
...
test('uploadCanMessage successfully logs message', async () => {
  ...
  assert.strictEqual(result, true, 'uploadCanMessage should return true on success');
});
...
test('uploadCanMessage handles failure gracefully', async () => {
  ...
  const result = await uploadCanMessage(data);
  assert.strictEqual(result, true, 'uploadCanMessage should return false on failure');
});
